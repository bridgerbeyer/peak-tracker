    ...
    <div style={{ minHeight: '100vh', background: '#F5F3EE', fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '1.5rem' }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '3rem', color: '#7A756E', fontSize: 14 }}>No units found.</div>
        ) : (
      ...
      {selectedUnit && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', zIndex: 50, display: 'flex', justifyContent: 'flex-end' }} onClick={() => setSelectedUnit(null)}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#fff', width: '100%', maxWidth: 560, height: '100%', overflowY: 'auto', boxShadow: '-4px 0 24px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid #E2DDD6', position: 'sticky', top: 0, background: '#fff', zIndex: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                ...
                <button onClick={() => setSelectedUnit(null)} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: '#7A756E' }}>×</button>
              </div>
                  ...
                  unitTasks(selectedUnit.id).map(task => (
                    <div key={task.id} style={{ borderBottom: '1px solid #F5F3EE', paddingBottom: 14, marginBottom: 14 }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                        <input type="checkbox" checked={task.completed} onChange={e => toggleTask(task.id, e.target.checked)} style={{ width: 18, height: 18, marginTop: 2, accentColor: '#2B4D3F', flexShrink: 0, cursor: 'pointer' }} />
                        <div style={{ flex: 1 }}>
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
                              {task.images.map(img => (
                                <div key={img.id} style={{ position: 'relative' }}>
                                  <img src={img.image_data} onClick={() => setExpandedImg(img.image_data)} style={{ width: 64, height: 64, objectFit: 'cover', borderRadius: 8, border: '1px solid #E2DDD6', cursor: 'pointer' }} />
                ...
                {selectedUnitPhotos.length === 0 ? (
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
                    {selectedUnitPhotos.map(photo => (
                      <div key={photo.id} style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', border: '1px solid #E2DDD6' }}>
                        ...
                        <div style={{ padding: '8px 10px', background: '#fff' }}>
                          {photo.caption && <div style={{ fontSize: 12, color: '#4B4640', marginBottom: 4 }}>{photo.caption}</div>}
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span style={{ fontSize: 11, color: '#7A756E' }}>{new Date(photo.created_at).toLocaleDateString()}</span>
                            <button onClick={() => deleteUnitPhoto(photo.id)} style={{ fontSize: 11, padding: '2px 6px', border: '1px solid #E2DDD6', borderRadius: 4, background: 'transparent', color: '#7A756E', cursor: 'pointer' }}>Remove</button>
                          </div>
                        </div>
                      </div>
                    ))}
