      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '1.5rem' }}>
        ...
        {tab === 'dashboard' && (
          <div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
              ...
              <button onClick={() => setTab('log')} style={{ flex: 1, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px', fontSize: 14, fontWeight: 500, cursor: 'pointer', color: 'var(--text)' }}>Phase improvements</button>
        ...
        {tab === 'log' && (
          <div style={{ background: 'var(--surface)', borderRadius: 16, border: '1px solid var(--border)', padding: '1.5rem' }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--gray)', marginBottom: 20 }}>Document what to do differently — builds your playbook for future phases.</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
              <div><label style={{ fontSize: 12, color: 'var(--gray)', display: 'block', marginBottom: 5 }}>Trade *</label>
              ...
              <div><label style={{ fontSize: 12, color: 'var(--gray)', display: 'block', marginBottom: 5 }}>Phase</label>
                <select value={phase} onChange={e => setPhase(e.target.value)} style={S.input}>{PHASES.map(p => <option key={p}>{p}</option>)}</select></div>
            </div>
            <div style={{ marginBottom: 12 }}><label style={{ fontSize: 12, color: 'var(--gray)', display: 'block', marginBottom: 5 }}>Short title (optional)</label>
              <input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Corner backing for drywall" style={S.input} /></div>
            <div style={{ marginBottom: 12 }}><label style={{ fontSize: 12, color: 'var(--gray)', display: 'block', marginBottom: 5 }}>Related unit (optional)</label>
              ...
              <div style={{ fontSize: 11, color: 'var(--gray)', marginTop: 4 }}>Link this improvement to a specific unit to avoid repeat notes.</div>
            </div>
            <div style={{ marginBottom: 12 }}><label style={{ fontSize: 12, color: 'var(--gray)', display: 'block', marginBottom: 5 }}>Description *</label>
            ...
            <div style={{ marginBottom: 20 }}><label style={{ fontSize: 12, color: 'var(--gray)', display: 'block', marginBottom: 5 }}>Photos</label>
              <div onClick={() => fileRef.current?.click()} style={{ border: '1px dashed #C4BFB8', borderRadius: 8, padding: '1rem', textAlign: 'center', cursor: 'pointer', fontSize: 13, color: 'var(--gray)' }}>Tap to add photos<input ref={fileRef} type="file" multiple accept="image/*" onChange={handlePhotos} style={{ display: 'none' }} /></div>
              {photos.length > 0 && <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>{photos.map((p, i) => <div key={i} style={{ position: 'relative' }}><img src={p} style={{ width: 72, height: 72, objectFit: 'cover', borderRadius: 8, border: '1px solid var(--border)' }} /><button onClick={() => setPhotos(prev => prev.filter((_, j) => j !== i))} style={{ position: 'absolute', top: -6, right: -6, width: 18, height: 18, borderRadius: '50%', background: '#991B1B', color: '#fff', border: 'none', fontSize: 10, cursor: 'pointer' }}>x</button></div>)}</div>}
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
              <button onClick={() => { setTrade(''); setPhase('Phase 1'); setDescription(''); setPhotos([]); setTitle('') }} style={{ padding: '9px 18px', border: '1px solid var(--border)', borderRadius: 8, background: 'transparent', fontSize: 14, color: 'var(--gray)', cursor: 'pointer' }}>Clear</button>
              <button onClick={submitEntry} disabled={submitting || !trade || !description.trim()} style={{ padding: '9px 20px', border: 'none', borderRadius: 8, background: 'var(--red)', color: '#fff', fontSize: 14, fontWeight: 500, cursor: 'pointer', opacity: (!trade || !description.trim()) ? 0.5 : 1 }}>
        ...
        {tab === 'library' && (
          <div>
            ...
            {filtered.length === 0 ? <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--gray)', fontSize: 14 }}>No improvements logged yet.</div>
              : Object.entries(tradeGroups).map(([tradeName, items]) => (
                <div key={tradeName} style={{ marginBottom: 24 }}>
                  ...
                  {items.map(entry => (
                    <div key={entry.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '1rem 1.25rem', marginBottom: 8, marginLeft: 18 }}>
                      ...
                      <div style={{ fontSize: 13, color: 'var(--text2)', lineHeight: 1.6, marginBottom: entry.ai_insight ? 10 : 0 }}>{entry.description}</div>
                      {entry.photos?.length > 0 && <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>{entry.photos.map((p, i) => <img key={i} src={p} style={{ width: 68, height: 68, objectFit: 'cover', borderRadius: 8, border: '1px solid var(--border)' }} />)}</div>}
                      {entry.ai_insight && <div style={{ background: 'var(--bg)', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: 'var(--text2)', lineHeight: 1.6, borderLeft: '3px solid var(--red)' }}><div style={{ fontSize: 10, fontFamily: 'DM Mono, monospace', letterSpacing: '0.08em', color: 'var(--gray)', marginBottom: 4 }}>AI SPEC NOTE</div>{entry.ai_insight}</div>}
                    </div>
                  ))}
                </div>
              ))}
